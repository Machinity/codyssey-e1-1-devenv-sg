...
🔲 **미션 목표 요약 (2~4문장)**
> 예: 이 과제는 리눅스 CLI, Docker, Git/GitHub을 직접 세팅하고 사용하여 "내 컴퓨터에서만 되는" 문제를
...
| 항목 | 값 |
|---|---|
| OS | 🔲 (예: macOS 15.x / Ubuntu 22.04) |
| Shell / Terminal | 🔲 (예: zsh, iTerm2) |
| 컨테이너 런타임 | 🔲 (예: OrbStack — 서울캠퍼스 sudo 제약으로 Docker Desktop 대신 사용) |
| Docker 버전 | 🔲 `docker --version` 결과 붙여넣기 |
| Git 버전 | 🔲 `git --version` 결과 붙여넣기 |
...
- [ ] 터미널 기본 조작 및 폴더 구성 (`docs/01-terminal-log.md`)
- [ ] 파일/디렉터리 권한 변경 실습 (`docs/02-permission-log.md`)
- [ ] Docker 설치 및 기본 점검 (`docs/03-docker-install-log.md`)
- [ ] Docker 기본 운영 명령 (images/ps/logs/stats) (`docs/04-docker-ops-log.md`)
- [ ] hello-world / ubuntu 컨테이너 실행 실습 (`docs/05-container-run-log.md`)
- [ ] Dockerfile 기반 커스텀 이미지 제작 (`docs/06-custom-image-log.md`, `app/Dockerfile`)
- [ ] 포트 매핑 접속 증거 2회 이상 (`docs/07-port-mapping-log.md`, `screenshots/port-mapping/`)
- [ ] 바인드 마운트 반영 증거 (`docs/08-bind-mount-log.md`, `screenshots/bind-mount/`)
- [ ] Docker 볼륨 영속성 증거 (`docs/09-volume-log.md`, `screenshots/volume/`)
